...
### 1b 深度模型 3 次实跑复测（GRU/LSTM/GATs/KRNN）

> 第 1b 节的 7 个 benchmarks 深度模型原为单次复用值（来自 `_deep_results.json`）。此处按批4方式（`n_jobs=0`、每个跑 3 次）实测，取均值±std，验证结果稳定性。含成本口径、csi300、TopkDropout(50,5)。

| 模型 | 数据集 | IC(均值±std) | Rank IC | 年化(含成本,均值±std) | IR(含成本) | MDD | n_ok |
|------|--------|-------------|---------|----------------------|-----------|-----|------|
| GRU | Alpha360 | 0.0485±0.0050 | 0.0564 | **0.0818±0.0428** | **1.0826** | -0.0920 | 3 |
| GATs | Alpha360 | 0.0483±0.0012 | 0.0598 | 0.0725±0.0286 | 0.9684 | -0.0706 | 3 |
| GATs | Alpha158 | 0.0466±0.0013 | 0.0539 | **0.1289±0.0172** | **1.7558** | -0.0887 | 3 |
| LSTM | Alpha360 | 0.0434±0.0007 | 0.0537 | 0.0257±0.0062 | 0.3556 | -0.1084 | 3 |
| GRU | Alpha158 | 0.0283±0.0002 | 0.0423 | 0.0229±0.0202 | 0.3321 | -0.1157 | 3 |
| LSTM | Alpha158 | 0.0271±0.0018 | 0.0411 | 0.0213±0.0083 | 0.3251 | -0.1038 | 3 |
| KRNN | Alpha360 | 0.0148±0.0069 | 0.0249 | -0.0672±0.0418 | -0.7929 | -0.3086 | 3 |

**与原 1b 复用单次值对比**：3 次实跑均值与 `_deep_results.json` 原单次值**在 IC 上高度一致**（如 GATs/Alpha158 0.0466 vs 0.0443、GRU/Alpha360 0.0485 vs 0.0495、GRU/Alpha158 0.0283 vs 0.0279），且 std 很小（IC std 多在 0.001~0.005），说明 benchmarks 原结果可复现、模型训练稳定；**年化收益的波动明显更大**（如 GRU/Alpha360 年化 std 达 0.0428、GATs/Alpha360 0.0286），因回测收益经 TopkDropout 放大后对随机 seed 比 IC 更敏感——这也印证了看单次年化易被随机性误导、多次取均值更可靠。GATs/Alpha158 仍是全场单模型收益冠军（12.89%、IR 1.76），KRNN/Alpha360 仍垫底（含成本 -6.72%，与原 -2.21% 同为负，3 次均确认其在 csi300 上失效）。7 个 config 全部 done，无失败/跳过。
